...
public class ChangeTextRuntimeCMDPMT : MonoBehaviour
{
        ...
        if (windowPtr != IntPtr.Zero)
        {
            if (textTimer >= textList[textLineCounter].m_waitTime)
            {
                ...
                if (textLineCounter >= textList.Count)
                {
                    ...
                    GetComponent<ChangeTextRuntimeCMDPMT>().enabled = false;
                }
            }
        ...
        else
        {
            ...
            EnumDelegate filter = delegate (IntPtr hWnd, int lParam)
            {
                ...
                if (IsWindowVisible(hWnd) && string.IsNullOrEmpty(strTitle) == false)
                {
                    ...
                    if (strTitle.Contains("cmd.exe"))
                    {
                        //"C:\\Windows\\System32\\cmd.exe"
                        windowPtr = FindWindowByCaption(IntPtr.Zero, strTitle);
                        textWindowPtr = FindWindowEx(windowPtr, IntPtr.Zero, null, null);
                    }
                }
                return true;
            };
